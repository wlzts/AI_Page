(function(){
  ...
  const npcs=[
    ...
    {name:'诺瓦',role:'实验室工程师',portrait:'assets/images/portrait-nova.png',variant:4,x:1510,y:670,dir:'left',frame:0,animT:0,speed:38,path:[[1510,670],[1370,670],[1370,710],[1510,710]],pi:0,lines:[`项目实验室目前收录 ${D.projects.length} 个核心项目。`,'重点不是项目数量，而是架构设计、数据处理、性能优化和最终落地。']},
  ...
  function interiorInteractions(scene){
    ...
    if(scene==='projects')return [exit,...D.projects.slice(0,6).map((p,n)=>{const pts=[[175,285],[410,245],[635,245],[845,285],[365,455],[665,455]];return{x:pts[n][0],y:pts[n][1],r:80,label:`查看项目：${p.title}`,action:()=>showProject(n)}})];
